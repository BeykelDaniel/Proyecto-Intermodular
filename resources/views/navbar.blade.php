...
<style>
    .flatpickr-calendar { background: #fdf6e9 !important; border: 2px solid #32424D !important; width: 320px !important; }
    .flatpickr-day { font-size: 1.1rem !important; font-weight: bold !important; height: 45px !important; line-height: 45px !important; }
    .dia-resaltado { position: relative; font-weight: 900 !important; color: #32424D !important; }
    .dia-resaltado::after {
        content: ""; position: absolute; bottom: 8px; left: 10%; right: 10%; height: 8px; 
        opacity: 0.5; border-radius: 4px; background-color: var(--color-actividad, #bc6a50); z-index: -1;
    }
    ...
    @media (max-width: 1024px) {
        ...
        .dropdown-content { width: 90% !important; left: 50% !important; transform: translateX(-50%) !important; }
    }
</style>

<nav class="bg-[#ebd08e] shadow-md h-24 md:h-28 flex items-center relative border-b-2 border-[#32424D]/10 px-6 md:px-12">
        ...
        <ul id="nav-menu" class="flex flex-row items-center gap-x-6 md:gap-x-10 lg:gap-x-14 font-bold">
            ...
            <li class="relative dropdown-container" x-data="{ dropdown: false }">
                <button @click="dropdown = !dropdown; if(dropdown) setTimeout(() => window.fpInstancia.redraw(), 50)" 
                    ...
                    Mis Actividades
                    <svg class="w-4 h-4 ms-2" :class="{'rotate-180': dropdown}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="dropdown" @click.away="dropdown = false" x-transition x-cloak 
                    ...
                    <div class="mt-4 border-t-2 pt-4">
                        ...
                        <ul id="lista-navbar-inscripciones" class="space-y-3 max-h-[200px] overflow-y-auto">
                            <li id="no-hay-actividades" class="text-sm text-gray-400 italic text-center">Aún no tienes planes</li>
                        </ul>
                    </div>
                </div>
            </li>

            <li><a href="#" class="text-[#32424D] uppercase whitespace-nowrap text-xs md:text-base hover:text-[#f4f6f6] transition-colors">Comunidades</a></li>
            <li><a href="#" class="text-[#32424D] uppercase whitespace-nowrap text-xs md:text-base hover:text-[#f4f6f6] transition-colors">Nosotros</a></li>
            <li><a href="#" class="text-[#32424D] uppercase whitespace-nowrap text-xs md:text-base hover:text-[#f4f6f6] transition-colors">Amigos</a></li>
            
            @auth
                <li>
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="text-[#82aeb4] font-black uppercase whitespace-nowrap text-xs md:text-base hover:scale-105 transition-transform">Salir</button>
                    </form>
                </li>
            @else
                <li><a href="{{ route('pagina.login_usuarios') }}" class="text-[#32424D] uppercase whitespace-nowrap text-xs md:text-base hover:text-[#f4f6f6] transition-colors">Entrar</a></li>
            @endauth
        </ul>
    </div>
</nav>
