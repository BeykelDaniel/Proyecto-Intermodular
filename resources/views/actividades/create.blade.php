    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                {{ $oper == 'create' ? 'Crear Nueva Actividad' : ($oper == 'edit' ? 'Editar Actividad' : 'Detalles') }}
            </h2>
            <a href="{{ route('actividades.index') }}" class="px-4 py-2 bg-white border rounded-lg font-semibold text-xs text-gray-700 uppercase shadow-sm hover:bg-gray-50">
                Volver
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12 bg-gray-50/50">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl border border-gray-100">
                ...
                <div class="p-8">
                    {{-- Alerta de Errores (Aparecerán los 5 errores que mencionaste) --}}
                    @if ($errors->any())
                    ...
                    <form action="{{ $oper == 'create' ? route('actividades.store') : route('actividades.update', $actividad->id) }}" method="POST">
                        @csrf
                        @if($oper == 'edit') @method('PUT') @endif

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                            @php
                                $inputClasses = "mt-2 block w-full px-4 py-3 bg-gray-50 border-gray-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200";
                                $labelClasses = "block text-sm font-bold text-gray-700 tracking-wide ml-1";
                            @endphp

                            {{-- 1. NOMBRE (required|string|max:255) --}}
                            ...
                            {{-- 6. PRECIO (required|numeric) --}}
                            <div>
                                <label class="{{ $labelClasses }}">Precio (€)</label>
                                <input type="number" step="0.01" name="precio" value="{{ old('precio', $actividad->precio ?? '') }}"
                                    class="{{ $inputClasses }}" {{ $oper=='show' ? 'disabled' : 'required' }}
                                    placeholder="0.00">
                            </div>

                            {{-- 7. CUPOS (required|numeric) --}}
                            <div>
                        ...
                        <div class="mt-12 pt-8 border-t border-gray-100 flex justify-end">
                            ...
                            @else
                                <a href="{{ route('actividades.edit', $actividad->id) }}" class="bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-10 rounded-xl shadow-lg">
                                    Editar Actividad
                                </a>
                            @endif
                        </div>
